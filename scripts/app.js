...
function renderizarProductos(productos) {
    ...
    productos.forEach((producto) => {
        ...
        <img src="${producto.imagen}" class="card-img-top card-img" alt="${producto.nombre}">
            <div class="card-body">
...
function actualizarContenidoCarrito(){
    ...
    carrito.forEach(producto=>{
        ...
        <td>
                <button class="btn btn-link text-danger" onclick="eliminarProducto('${producto.nombre}')">
...
function actualizarTotalGeneralPrecios() {
    const totalPrecios = carrito.reduce((total, producto) => total + (producto.precio * producto.cantidad), 0);
...
document.querySelector(`#miModal .modal-footer .btn-success`).addEventListener(`click`, realizarCompra);
...
function filtrarProductos() {
    ...
    const productos = document.getElementById('productos-container').getElementsByClassName('card');
